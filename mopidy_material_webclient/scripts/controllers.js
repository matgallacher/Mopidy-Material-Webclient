...
controllers.controller('PlayingCtrl', [
    '$scope', 'mopidy', 'lastfm',
    function ($scope, mopidy, lastfm) {
        $scope.loading = true;
        ...
        $scope.getInfo = function (track) {
            ...
            lastfm.getTrack(track).success(function (trackInfo) {
                ...
                if (t.album
                    && t.album.image
                    && t.album.image.length > 0) {
                    $scope.playing.album = t.album;
                    $scope.playing.album.images = [];
                    for (var i = 0; i < t.album.image.length; i++) {
                        $scope.playing.album.images.push(t.album.image[i]['#text']);
                    }
                }
            });
        };
...
controllers.controller('PlaylistsCtrl', [
    ...
    function ($scope, $routeParams, $location, mopidy) {
        ...
        $scope.goTo = function (ref) {
            if ($scope.isPlaylist) {
                $scope.mopidy.tracklist.clear();
                $scope.mopidy.tracklist.add(null, null, ref.uri)
                    .then(function () {
                        $scope.mopidy.playback.play();
                    });
            } else {
...
controllers.controller('QueueCtrl', [
    '$scope', '$mdDialog', '$mdToast','mopidy',
    function ($scope, $mdDialog, $mdToast, mopidy) {
        mopidy.then(function (m) {
            $scope.mopidy = m;
